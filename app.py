...
def catalog():
    ...
    try:
        ...
        if not (country_kz and country_ru and country_cn):
            # Если не все страны выбраны, нужна фильтрация
            country_conditions = []
            
            if country_cn:
                country_conditions.append("sr.marketplace ILIKE ANY(ARRAY['%1688%', '%taobao%', '%temu%', '%aliexpress%', '%pinduoduo%'])")
            if country_kz:
                country_conditions.append("sr.marketplace ILIKE ANY(ARRAY['%kaspi%', '%satu%', '%ozon.kz%', '%otevertka%'])")
            if country_ru:
                country_conditions.append("sr.marketplace ILIKE ANY(ARRAY['%wildberries%', '%ozon.ru%', '%ozon%', '%chipdip%', '%wb.ru%'])")
        ...
        # Подсчет общего количества с учетом фильтров по странам
        if not (country_kz and country_ru and country_cn) and (country_kz or country_ru or country_cn):
            # Нужна фильтрация по странам
            cursor.execute(f"""
                SELECT COUNT(DISTINCT l.id) as total 
                FROM lots l
                INNER JOIN search_results sr ON l.lot_number = sr.lot_number
                WHERE {where_sql}
            """, params)
        else:
            # Все страны или ни одной - показываем все
            cursor.execute(f"""
        ...
        if not (country_kz and country_ru and country_cn) and (country_kz or country_ru or country_cn):
            # С фильтрацией по странам
            cursor.execute(f"""
                SELECT l.*, 
                       COUNT(DISTINCT sr.id) as products_count,
                       MIN(CAST(NULLIF(regexp_replace(sr.product_price, '[^0-9.]', '', 'g'), '') AS DECIMAL)) as min_price
                FROM lots l
                INNER JOIN search_results sr ON l.lot_number = sr.lot_number
                WHERE {where_sql}
                GROUP BY l.id
...
def lot_detail(lot_number):
    ...
    try:
        ...
        for product in products:
            ...
            # Китай: 1688, TaoBao, Temu, AliExpress, Pinduoduo
            if any(x in marketplace for x in ['1688', 'taobao', 'temu', 'aliexpress', 'pinduoduo']):
                products_by_country['CN'].append(product)
            # Казахстан: Satu.kz, Kaspi.kz, Ozon.kz
            elif any(x in marketplace for x in ['kaspi', 'satu', 'ozon.kz', 'otevertka']):
                products_by_country['KZ'].append(product)
            # Россия: Wildberries, Ozon, Chipdip
            elif any(x in marketplace for x in ['wildberries', 'ozon.ru', 'ozon', 'chipdip', 'wb.ru']):
