...
def get_lots():
    try:
        ...
        limit = min(int(request.args.get('limit', 20)), 50)  # Меньше лотов = быстрее
        
        if search:
            cur.execute("""
                SELECT lot_number, simplified_name, original_name, tender_price, quantity, category
                FROM lots 
        ...
        # Получаем избранное одним запросом
        ...
        for lot in cur.fetchall():
            ...
            # Быстрые вычисления
            profit = (price * qty) * 0.54  # Упрощённая формула
            roi = 117.4 if price > 0 else 0  # Примерное значение
            
            lot_dict['stats'] = {'profit': round(profit, 2), 'roi': roi}
