...
def calculate_profitability(lot_number, tender_price, quantity, cur):
    ...
    try:
        ...
        for row in rows:
            ...
            price_value = row.get('product_price') if isinstance(row, dict) else row[0]
            if price_value:
        ...
        # Логируем ошибку без остановки выполнения
        import sys
        sys.stderr.write(f"Error calculating profitability for {lot_number}: {str(e)}\n")
